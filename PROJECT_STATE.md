...
- **148 tests** backend (`./run_tests.sh`), sauvegardes Postgres quotidiennes (`./backups/`), rotation des logs.
...
Ce qui dépend de toi : installer Tailscale (+ `tailscale serve`), installer l'app ntfy et choisir un topic. Idées ensuite : Web Push natif (une fois en HTTPS), Gantt avec déplacement des échéances, export PDF du briefing.
